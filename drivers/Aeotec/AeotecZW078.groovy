 ...
 *
 *  Copyright 2021 Elastic Development
 *
...
metadata {
  definition (
      ...
      author: 'James P',
      importUrl: 'https://raw.githubusercontent.com/jpansarasa/Hubitat/main/drivers/Aeotec/AeotecZW078.groovy'
    ) {
    capability 'Actuator'
    capability 'Switch'
    capability 'Sensor'
    ...
    fingerprint inClusters: '0x5E,0x86,0x72,0x98,0x56'
  ...
  preferences {
    ...
    section { // Configuration
      ...
      input name: 'param101_kwh', title: 'Report Accumulated kWh', description: '', type: 'bool', defaultValue: true, required: true
...
def initialize() {
  ...
  cmds = cmds + cmdSequence([
    ...
    zwave.configurationV1.configurationSet(parameterNumber: 255, size: 1, 1),
    zwave.configurationV1.configurationSet(parameterNumber: 252, size: 1, scaledConfigurationValue: 1)
  ], 500)
  configure()
}
...
def configure() {
  ...
  switch (switchAll) {
    ...
    case 'Disabled':
    default:
      switchAllMode = hubitat.zwave.commands.switchallv1.SwitchAllSet.MODE_INCLUDED_IN_THE_ALL_ON_ALL_OFF_FUNCTIONALITY
      break
  }

  Integer reportGroup = ("$param101_voltage" == 'true' ? 1 : 0)
  reportGroup += ("$param101_current" == 'true' ? 2 : 0)
  reportGroup += ("$param101_watts" == 'true' ? 4 : 0)
  reportGroup += ("$param101_kwh" == 'true' ? 8 : 0)
...
def parse(String description) {
  ...
  def cmd = zwave.parse(description, COMMAND_CLASSES)
...
def zwaveEvent(hubitat.zwave.commands.basicv1.BasicReport cmd) {
  ...
  if (logDescText) {
    log.info "${device.displayName} is ${value}"
  } else if (map?.descriptionText) {
    logger('info', "is ${value}")
  }

  result << createEvent(name: 'switch', value: value, descriptionText: "is ${value}")
  result
}
...
def zwaveEvent(hubitat.zwave.commands.securityv1.SecurityMessageEncapsulation cmd) {
  ...
  setSecured()
  def encapsulatedCommand = cmd.encapsulatedCommand(COMMAND_CLASSES)
